...
class SyncBlobStorage:
    ...
    def _read_sync_state_from_blob(self) -> Optional[Dict]:
        """Read sync state from existing blob with retry logic for 404/403 errors"""
        max_retries = 3
        
        for attempt in range(max_retries):
            try:
                if attempt > 0:
                    print(f"📋 Retry {attempt}/{max_retries-1}: Listing existing blobs...")
                    import time
                    ...
                    for blob in blobs_response['blobs']:
                        if blob.get('pathname') == self.sync_state_filename:
                            ...
                            try:
                                ...
                                print(f"📄 HTTP error reading blob (attempt {attempt+1}): {req_error}")
                                if ("404" in str(req_error) or "403" in str(req_error)) and attempt < max_retries - 1:
                                    print(f"📄 Blob access error - will retry in 1 second...")
                                    continue  # Try again with a fresh blob list
                                elif "403" in str(req_error) or "Forbidden" in str(req_error):
                ...
                print(f"📄 Error reading sync state (attempt {attempt+1}): {e}")
    ...
    def save_sync_state(self, sync_state: Dict) -> bool:
        ...
        try:
            ...
            if upload_result and 'url' in upload_result:
                ...
                # Add a small delay to allow blob storage to settle
                import time
                time.sleep(0.5)
