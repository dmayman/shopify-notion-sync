...
class ShopifyNotionSync:
    def __init__(self):
        ...
        self.sync_storage = SyncDatabaseStorage()
    ...
    def sync_orders_to_notion(self, mode='smart', limit=50):
        """Ultra-minimal sync function using Shopify's updatedAt field - batch database writes"""
        try:
            ...
            db_success = self.sync_storage.end_batch_mode()
            if not db_success:
                print("⚠️ Warning: Failed to save sync state to database")
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # Support limit from both query params and request body (query params take precedence)
            try:
                sync_limit = int(query_params.get('limit', request_data.get('limit', 50)))
                # Validate limit range (1-1000)
                if sync_limit < 1 or sync_limit > 1000:
                    raise ValueError(f"Limit must be between 1 and 1000, got: {sync_limit}")
